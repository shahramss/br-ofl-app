...
۹. توضیح متای یکتا بساز که بلافاصله بعد از نام محصول، تمام مشخصات فنی استخراج‌شده را با نام و مقدار بیاورد. هیچ مشخصه‌ای را حذف نکن، مگر اینکه تکراری یا خالی باشد. فقط مشخصات واقعی را بنویس و مقدار جدید حدس نزن. ساختار پیشنهادی: «خرید [نام محصول] با مشخصات کامل: قطر ...، طول ...، جنس ...؛ قیمت مناسب، خرید عمده و ارسال سریع.» متن متا برای همه محصولات شبیه هم نباشد، اما اولویت اصلی آوردن تمام مشخصات فنی است. تبلیغات دروغین، ارزان‌ترین، بهترین، تضمینی یا ادعای غیرقابل اثبات ننویس. متا می‌تواند کامل‌تر باشد و حداکثر ۹۰ کلمه و حداکثر ۵۰۰ کاراکتر داشته باشد.
خروجی فقط JSON معتبر باشد، بدون توضیح اضافه.
فرمت دقیق:
{"specs":[{"name":"قطر","value":"۱۰ میلی‌متر"},{"name":"جنس","value":"فولاد"}],"content":"این محصول با قطر ۱۰ میلی‌متر و جنس فولاد برای استفاده‌های فنی مرتبط با مشخصات اعلام‌شده مناسب است. با توجه به ابعاد و جنس گفته‌شده، می‌توان آن را برای انتخاب دقیق‌تر در زمان خرید و مقایسه مشخصات محصول بررسی کرد. در توضیحات این محصول فقط اطلاعاتی آمده که از متن ثبت‌شده استخراج شده و از افزودن ویژگی‌های نامشخص یا ادعاهای غیرواقعی خودداری شده است. این محصول با ارسال فوری از بازار قفل سفارش بدید","seo_title":"قیمت و خرید سنبه","seo_description":"خرید سنبه با مشخصات کامل: قطر ۱۰ میلی‌متر، جنس فولاد؛ مناسب بررسی دقیق مشخصات فنی محصول، خرید عمده و ارسال سریع."}
''';
    ...
    try {
      ...
      return AiExtractResult(specs: specs, content: _limitChars(productContent, 900), seoTitle: _limitChars(seoTitle, 60), seoDescription: _limitWords(_limitChars(seoDescription, 500), 90));
    } catch (e) {
      if (e is ApiException) rethrow;
  ...
  String _buildFallbackSeoDescription(String productName, List<SpecItem> specs) {
    ...
    final text = parts.isEmpty
        ...
        : 'خرید $name با مشخصات کامل: $parts؛ مناسب بررسی دقیق محصول، خرید عمده و ارسال سریع.';
    return _limitWords(_limitChars(text, 500), 90);
  }
                      ...
                      child: Column(
                        children: [
                          ...
                          TextField(
                            ...
                            decoration: const InputDecoration(labelText: 'عنوان سئو', hintText: 'فقط قیمت و خرید + نام محصول'),
                          ),
                          const SizedBox(height: 12),
                          TextField(
                            maxLength: 155,
                            ...
                            decoration: const InputDecoration(labelText: 'توضیح متا', hintText: 'بعد از نام محصول، تمام مشخصات فنی بیاید؛ حداکثر ۹۰ کلمه / ۵۰۰ کاراکتر'),
                          ),
                          const SizedBox(height: 16),
                          AppCard(
                            child: Column(
